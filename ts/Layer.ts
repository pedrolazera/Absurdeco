...
function create_layer(max_size: number,
	qtd_colors: number, map_i_to_j: number[]): T_Layer {
	let head = 0;
...
function update_layer(C: T_Layer, cor: number): void {
	_update_layer(C, C.p_heads[cor], C.p_sizes[cor]);
}

function update_layer_from_array(C: T_Layer, v: number[]): void {
...
function _update_layer(C: T_Layer, new_head: number,
	new_len: number): void {
	C.head = new_head
	C.len = new_len
}
...
function create_partition(C1: T_Layer, C2: T_Layer,
	...
	_j = C1.head;
	while(_j != _NULL_ID) {
		if(_i == _j) { _j = C1.nexts[_j]; continue; }
		color = colors[i][_j];
...
function is_in(i: number, C:T_Layer) {
	let _i = C.map_i_to_j[i];
	return (_i >= 0) && (C.p_ids[C.head] == C.p_ids[_i]);
}

function create_layers(qtd_layers: number, max_size: number,
	qtd_colors: number, map_i_to_j: number[]): T_Layer[] {
...
function create_map_i_to_j(N: number, G_ids: number[]): number[] {
	let M = G_ids.length;
...
function _layer_vectorize(C: T_Layer) {
	let v = new Array();
...
function _layer_printa(C: T_Layer) {
